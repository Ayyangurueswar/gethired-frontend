...
const Page = () => {
  ...
  useEffect(() => {
    ...
    queryString += domain !== '' ? `&filters[domain][$containsi]=${domain}` : '';
    ...
    <div className="w-full overflow-y-auto">
        ...
        <form className="w-full px-14 flex items-end justify-center mt-24">
            <div className="w-full flex items-center justify-between">
                ...
                <input name="domain" placeholder="Search by domain" className="px-4 py-2 rounded-md border border-slate-700 outline-none" onChange={(e) => setDomain(e.target.value)}/>
                <div>
        ...
        <div className="w-full px-14 flex flex-col gap-6 my-10">
            {
                loading ? <p className="text-center text-3xl">Loading</p> : jobs.length > 0 ? jobs.map((job) => {
                    return (
                        <div key={job.id} className="w-full flex items-center justify-between px-6 py-4 shadow-md rounded-md shadow-slate-600 hover:scale-105 transition">
                            <div className="w-1/3">
                                <h3 className="text-2xl font-semibold">{job.attributes.title}</h3>
                                <p className="text-slate-500">at {job.attributes.company}</p>
                            </div>
                            <div className="w-2/5 flex items-center justify-between">
                                <p className="text-gray-500 w-3/4 overflow-ellipsis flex items-center"><Image src='/locaion-icon.png' className="inline-block mr-2" alt="" width={20} height={20}/><p className="overflow-hidden whitespace-nowrap text-ellipsis">{job.attributes.location}</p></p>
                                <p className="text-gray-500 w-1/2"><Image src='/icons8-receive-cash-50.png' className="inline-block mr-2" alt="" width={20} height={20}/>&#8377; {job.attributes.stipend}</p>
                            </div>
                            <Link href={`/jobs/view/${job.id}`} className="px-6 py-2 bg-slate-900 text-white rounded-md">View details</Link>
                        </div>
                    )
